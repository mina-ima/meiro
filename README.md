...
- `.github/workflows/deploy-preview.yml` は Pull Request 作成時に Workers/Pages のプレビューを発行します。
- `.github/workflows/deploy-production.yml` は `main` への push（または手動実行）で Cloudflare Workers の本番環境へ自動デプロイします。Typecheck とテストが通った場合のみ `wrangler deploy` を実行します。
  - 必要なシークレット: `CF_ACCOUNT_ID`, `CF_WORKERS_API_TOKEN`
  - 失敗時は `gh run rerun <run-id>` で再実行可能。手動デプロイにフォールバックする場合は `cd server && npx wrangler deploy` を使用してください。
