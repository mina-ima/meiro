...
* **スナップショット**：Phase遷移/編集確定時は全量、小刻み更新は**位置/スコア等の差分**。
* **サーバ→クライアント送信**は**20Hz**上限でバッチ。クライアント補間表示。
...
* 単位は**1辺=1本**。初期本数（20×20:48本 / 40×40:140本）。削除は**1回権利**で返却。
...
* **ミニマップなし**、**ヘッドボブなし**。HUD：残時間/現在ポイント/ゴール到達ボーナス値。
