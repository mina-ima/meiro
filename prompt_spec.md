...
* 探索フェーズ中のプレイヤービューは黒背景に**ASCIIスタイルのワイヤーフレーム透視図**を描き、すべて赤一色の廊下縁ライン・中心ガイド・点線グリッド・小さな長方形の開口部で進行方向と壁面を明確に示す。
...
* [x] 視界：FOV90°, 到達4マス（4マス目減光）＋迷路セルを2倍格子に投影してASCIIワイヤーフレームを動的描画（`client/tests/PlayerViewRaycaster.test.tsx`）
