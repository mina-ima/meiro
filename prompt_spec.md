...
* 探索フェーズ中のプレイヤービューは黒背景に**赤い1点透視ワイヤーフレーム**のみを描画し、左右の壁は均一な赤ドットのグリッド、中央は破線の小さな矩形開口部で表現する。床グロー/フォグ/多色テクスチャは禁止し、テクニカルなASCII風ビジュアルに統一する（`client/tests/PlayerViewRaycaster.test.tsx`）。
