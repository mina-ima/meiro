...
* ルータ↔DOハンドシェイク：`/ws` ハンドラは `Upgrade: websocket` を必須とし、`WebSocketPair` の片方を `RoomDO.fetch("/session")` へ渡す。DO 側は `/session` で再び `Upgrade` を検証して `webSocket.accept()` を呼び、即座に `DEBUG_CONNECTED` → `STATE(full)` を送出して接続成立をログ（`WS fetch /ws` / `DO connected` / `send DEBUG_CONNECTED` / `send STATE`）に残す。Cloudflare 環境では Worker→DO の `stub.fetch()` が `GET` として届くケースがあるため、`/session` は `GET`/`POST` どちらでも `Upgrade: websocket` が付与されていれば受け入れ、DO が 101 以外の HTTP ステータスを返した場合はルータが両ソケットを `close(1011, "upgrade rejected")` して DO 応答（409/410/500等）をそのままクライアントへ返却する。
* Cloudflare の DO ルーティングでは `request.url.pathname` に `/<DurableObjectId>/...` のプレフィックスが付与される。`RoomDO.fetch()` では `url.pathname.endsWith("/session")` / `endsWith("/rematch")` のように末尾一致で判定し、いずれの場合も `webSocket.accept()` 後に **必ず 101** を返す。
