...
- 更新メモ(2025-12-08): FancyMazePreview の分岐床/壁を通路投影関数に統一し、slice2 基準で床が連続し壁が手前に食い込まないよう再調整（`client/tests/fancyMazePreview.test.tsx` で床/壁連続性を検証）
...
- 更新メモ(2026-01-12): FancyMazePreview junction の分岐床 near 側を slice2 床稜線(anchorY)の角(anchorX)から開始し、内側壁の起点も同じ座標に揃えて「外した slice2 壁の奥から壁が生える」ことをテストで回帰（`client/tests/fancyMazePreview.test.tsx` に anchorX 検証を追加）
...
- 更新メモ(2026-01-21): FancyMazePreview junction/goal をスライスベースの描画に戻し、slice2 anchorX/anchorY から分岐床と内外壁を描画。描画順を床→壁→分岐→前壁にそろえ、`client/tests/fancyMazePreview.test.tsx` で anchorY と inner wall 起点の一致を確認
- 更新メモ(2026-01-23): FancyMazePreview junction/goal の分岐床 nearInner/nearOuter を本線 slice2 床角と同座標に固定し、内側壁の起点も nearInner と共有。openings 側の slice2 壁非表示を維持しつつ、branch 床/内壁が本線床稜線から連続して伸びることを `client/src/views/FancyMazePreview.ts` / `client/tests/fancyMazePreview.test.tsx` で厳密化
...
- 更新メモ(2026-02-05): FancyMazePreview junction/goal に mouth インセットを導入し、nearInner を通路内側へ押し込んで手前エッジを線分として接続。内外壁の根元を nearInner/nearOuter に合わせ、branch clip を分岐側だけの台形に簡略化、シームを main内側→アンカー→branch nearOuter→nearInner の4点に整理（`client/src/views/FancyMazePreview.ts` / `client/tests/fancyMazePreview.test.tsx` で mouth インセットと壁根元・シーム・クリップを検証）
- 更新メモ(2026-02-06): FancyMazePreview junction/goal で mouth インセットとシームを撤去し、anchorStop 上に mouthWidth=anchorWidth*0.12 の共有エッジを直接描画。branch-opening-fill とマスクのみを残して clipPath を無効化し、分岐壁の根元も mouth 線分に合わせて手前飛び出しを抑制。描画順は 床→床スライス→分岐床→開口フィル→defs→メイン壁→分岐壁→キャップ に固定（`client/src/views/FancyMazePreview.ts` / `client/tests/fancyMazePreview.test.tsx` で床連続性とclip非使用を検証）
- 更新メモ(2026-02-07): FancyMazePreview junction/goal の BRANCH_ANCHOR を slice1 深度(1.0)に戻し、branch wall mask の開口スライスを slice2 のみに限定（slice1 は含めない）。床用 clipPath は anchorStop.y と mouthWidth に揃えて連続面を維持し、`client/src/views/FancyMazePreview.ts` / `client/tests/fancyMazePreview.test.tsx` で mask スライス集合とクリップ位置を検証
...
- [x] junction/goal の branch アンカーを slice1 手前へ繰り上げ、開口が手前から抜けて柱が残らないようにする  
